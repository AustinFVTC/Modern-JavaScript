...
document.getElementById("getFile").onchange = function() {
      ...
      let userFile = this.files[0];  // Get the first (and likely only) file submitted

      try {
            let isText = userFile.type.startsWith("text");
            if (!isText) throw userFile.name + " is not a text file!";
            ...
            // Once the file has finished loading, display it in the page
      ...
      function wordCloud(sourceText) {
            ...
            for (let i = 1; i < words.length; i++) {
                  if (words[i] === words[i-1]) {
                        // Duplicate, so increase frequency
            ...
            // Able to pass in a custome function to sort
            // Sort by descending order of frequency
            unique.sort(byDuplicate);

            /** Sort by descending order of unique word frequency */
            function byDuplicate(a, b) {
                  return b[1]-a[1];
            }

            // Keep the top 100 used words
            ...
            // Size each word based on its usage frequency
...
-------------------------------------------------
> Also WOW markdown works in these JavaDocs!
 */
