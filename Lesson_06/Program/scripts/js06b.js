...
function validateName() {
   /** Input field container the users Name as written on their Card */
...
/** Check if a credit card type has been selected */
...
function validateNumber() {
   ...
   if (cNum.validity.valueMissing) {
      cNum.setCustomValidity("Please enter your card number");
...
/** Checks the user selected an expiration month */
...
/** Checks the user selected an expiration year */
...
function validateCVC() {
   ...
   if (cvc.validity.valueMissing) {
      cvc.setCustomValidity("Please enter your CVC number on the back of your card");
   } else if ((card === "amex") && !(regx4Dig.test(cvc.value))) {
      cvc.setCustomValidity("Enter a 4-digit number");
   } else if ((card !== "amex") && !(regx3Dig.test(cvc.value))) {
